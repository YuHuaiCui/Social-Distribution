...
export class InboxService extends BaseApiService {
  ...
  async getInbox(
    ...
    return this.request<PaginatedResponse<InboxItem>>(
  ...
  async getInboxItem(id: string): Promise<InboxItem> {
    ...
    return this.request<InboxItem>(`/api/inbox/${inboxId}/`);
  }
  ...
  async markItemAsRead(id: string): Promise<InboxItem> {
    // Extract ID from URL if full URL is passed
    const inboxId = id.includes("/") ? id.split("/").filter(Boolean).pop() : id;
    return this.request<InboxItem>(`/api/inbox/${inboxId}/read/`, {
      method: "POST",
    });
  }
  ...
  async deleteItem(id: string): Promise<void> {
    // Extract ID from URL if full URL is passed
    const inboxId = id.includes("/") ? id.split("/").filter(Boolean).pop() : id;
    await this.request(`/api/inbox/${inboxId}/`, {
      method: "DELETE",
    });
  }
  ...
  async getFollowRequests(
    params?: Omit<InboxFilterParams, "content_type">
  ): Promise<PaginatedResponse<InboxItem>> {
