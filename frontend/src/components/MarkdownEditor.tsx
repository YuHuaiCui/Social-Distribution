...
import { 
  ...
  Eye, Maximize2, Minimize2, HelpCircle,
  FileText, Split, Undo, Redo
} from 'lucide-react';
...
export const MarkdownEditor: React.FC<MarkdownEditorProps> = ({
  ...
  const renderMarkdown = (markdown: string) => {
    ...
    let html = markdown
      ...
      .replace(/^# (.*$)/gim, '<h1 class="text-3xl font-bold mb-4 text-text-1">$1</h1>')
      // Bold
      ...
      // Blockquotes
      .replace(/^> (.*$)/gim, '<blockquote class="border-l-4 border-[var(--primary-violet)] pl-4 py-2 my-4 text-text-2">$1</blockquote>')
      // Lists
  ...
  return (
    <div 
      className={`${className} ${isFullscreen ? 'fixed inset-0 z-50 bg-bg-1' : ''}`}
    >
      {/* Toolbar */}
          ...
          <div className="flex items-center space-x-1">
            {toolbarButtons.map((button) => {
              ...
              return (
                ...
                >
                  <Icon size={18} className="text-text-2 group-hover:text-text-1" />
                </motion.button>
              );
            })}
      ...
      >
        <div className={`flex h-full ${viewMode === 'split' ? 'divide-x divide-border-1' : ''}`}>
          {/* Editor */}
      ...
      <AnimatePresence>
        {showHelp && (
          <>
            ...
            >
              <h3 className="text-xl font-semibold text-text-1 mb-4">Markdown Cheat Sheet</h3>
...
2. Another item`}
                ...
                <div>
                  <h4 className="font-medium text-text-1 mb-2">Links & Images</h4>
                  <pre className="bg-glass-low rounded p-2 text-text-2">
