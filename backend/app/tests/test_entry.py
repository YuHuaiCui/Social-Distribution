...
class EntryAPITest(BaseAPITestCase):
    ...
    def test_entry_list(self):
        ...
        url = reverse('social-distribution:author-entries-list')
        ...
        self.assertEqual(response.data['results'][0]['content'], 'This is a private entry')
        ...
        self.assertEqual(response.data['results'][1]['content'], 'This is a public entry')
    ...
    def test_entry_detail(self):
        ...
        url = reverse('social-distribution:author-entries-detail', args=[self.public_entry.id])
        ...
        url = reverse('social-distribution:author-entries-detail', args=[self.private_entry.id])
        response = self.user_client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        url = reverse('social-distribution:author-entries-detail', args=[self.private_entry_2.id])
        response = self.user_client.get(url)
        # Im not sure if we should get a 404 or a 403 here
        self.assertIn(response.status_code, [status.HTTP_403_FORBIDDEN, status.HTTP_404_NOT_FOUND])

        # Test non-existent entry
        url = reverse('social-distribution:author-entries-detail', args=[uuid.uuid4()])
        response = self.user_client.get(url)
    ...
    def test_entry_update(self):
        ...
        self.assertEqual(response.data['title'], 'Updated Title')  # Should remain from previous update
    ...
    def test_entry_delete(self):
        ...
        url = reverse('social-distribution:author-entries-detail', args=[non_existent_id])
        response = self.user_client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
