...
class InboxReceiveView(APIView):
    ...
    def post(self, request, author_id=None):
        ...
        try:
            ...
            print(
                f"DEBUG: Received inbox request for author {author_id}"
                if author_id
                else "DEBUG: Received public inbox request"
            )
            ...
            if author_id is None:
                ...
                author = Author.objects.filter(
                    node__isnull=True, is_active=True
                ).first()
                if not author:
                    print("DEBUG: No local authors found for general broadcast")
                    return Response(
                        {"error": "No local authors available"},
                ...
                try:
                    ...
                    print(
                        f"DEBUG: Found author: {author.username} (local: {author.is_local})"
                    )
                except Author.DoesNotExist:
                    # Try to find the author by other means (URL, username, etc.)
                    print(
                        f"DEBUG: Author {author_id} not found, attempting to find by other means"
                    )
                    ...
                    for local_author in local_authors:
                        print(
                            f"DEBUG: - {local_author.username} (ID: {local_author.id}, URL: {local_author.url})"
                        )
            ...
            if author_id is None and auth_header and auth_header.startswith("Basic "):
                try:
                    ...
                    if node:
                        print(
                            f"DEBUG: Accepting PUBLIC post from any node (using {node.name} as placeholder)"
                        )
                    else:
                        print("DEBUG: No active nodes configured")
                        return Response(
                            {"error": "No active nodes configured"},
            ...
            # For specific author inboxes, require proper authentication
                ...
                try:
                    ...
                    for node_obj in all_nodes:
                        print(
                            f"DEBUG: Node - Name: {node_obj.name}, Host: {node_obj.host}, Username: {node_obj.username}, Active: {node_obj.is_active}"
                        )
                    ...
                    try:
                        ...
                        print(
                            f"DEBUG: Checking if username exists but password doesn't match..."
                        )
                        try:
                            node_with_username = Node.objects.get(
                                username=username, is_active=True
                            )
                            print(
                                f"DEBUG: Found node with username but password doesn't match: {node_with_username.name}"
                            )
                        except Node.DoesNotExist:
            ...
            print(f"DEBUG: ==========================================")
            return Response(
                {"error": f"Failed to process object: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def _handle_follow_request(self, recipient, data, remote_node):
        ...
        if not actor_url:
            return Response(
                {"error": "Actor URL required"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Get or create the remote author
        remote_author, created = self._get_or_create_remote_author(
            actor_data, remote_node
        )

        # Check if follow request already exists
        existing_follow = Follow.objects.filter(
            follower=remote_author, followed=recipient
        ).first()
        ...
        Inbox.objects.create(
            recipient=recipient, item_type=Inbox.FOLLOW, follow=follow, raw_data=data
        )

        return Response(
            {"message": "Follow request received"}, status=status.HTTP_200_OK
        )
        ...
        if isinstance(actor_data, str):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise Exception(
                        f"Failed to fetch actor data: {response.status_code}"
                    )
            except Exception as e:
    ...
    def _handle_like(self, recipient, data, remote_node):
        ...
        try:
            ...
            print(
                f"DEBUG: Remote author: {remote_author.username} (created: {created})"
            )
            ...
            try:
                if isinstance(object_url, str):
                    ...
                    try:
                        ...
                        object_type = "entry"
                        print(
                            f"DEBUG: Found liked entry by exact URL: {liked_object.title}"
                        )
                        ...
                        for part in reversed(path_parts):
                            if part and part != "entries":
                                ...
                                try:
                                    ...
                                    object_type = "entry"
                                    print(
                                        f"DEBUG: Found liked entry by ID from URL: {liked_object.title}"
                                    )
                                    break
                        ...
                        if not liked_object:
                            ...
                            for part in reversed(path_parts):
                                if part and part != "entries":
                                    print(
                                        f"DEBUG: Trying to find entry with URL containing: {part}"
                                    )
                                    try:
                                        liked_object = Entry.objects.get(
                                            url__icontains=part
                                        )
                                        object_type = "entry"
                                        print(
                                            f"DEBUG: Found liked entry by URL contains: {liked_object.title}"
                                        )
                                        break
                                    except Entry.DoesNotExist:
                                        print(
                                            f"DEBUG: No entry found with URL containing: {part}"
                                        )
                                        continue
                else:
                    ...
                    try:
                        ...
                        object_type = "entry"
                        print(
                            f"DEBUG: Found liked entry by direct ID: {liked_object.title}"
                        )
                    except (Entry.DoesNotExist, ValueError):
                        pass
                ...
                try:
                    if isinstance(object_url, str):
                        ...
                        try:
                            ...
                            object_type = "comment"
                            print(
                                f"DEBUG: Found liked comment by exact URL: {liked_object.content[:50]}..."
                            )
                            ...
                            for part in reversed(path_parts):
                                if part and part != "comments":
                                    print(
                                        f"DEBUG: Trying to find comment with ID: {part}"
                                    )
                                    try:
                                        liked_object = Comment.objects.get(id=part)
                                        object_type = "comment"
                                        print(
                                            f"DEBUG: Found liked comment by ID from URL: {liked_object.content[:50]}..."
                                        )
                                        break
                                    except (Comment.DoesNotExist, ValueError):
                                        print(
                                            f"DEBUG: No comment found with ID: {part}"
                                        )
                                        continue
                    else:
                        # If object_url is not a string, try to find by ID directly
                        try:
                            ...
                            object_type = "comment"
                            print(
                                f"DEBUG: Found liked comment by direct ID: {liked_object.content[:50]}..."
                            )
                        except (Comment.DoesNotExist, ValueError):
                            pass
            ...
            if not liked_object:
                ...
                return Response(
                    {"message": "Liked object not found"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            print(
                f"DEBUG: - author: {remote_author.username} (URL: {remote_author.url})"
            )
            print(
                f"DEBUG: - entry: {liked_object.title if object_type == 'entry' else 'None'}"
            )
            print(
                f"DEBUG: - comment: {liked_object.content[:50] if object_type == 'comment' else 'None'}"
            )
            ...
            if object_type == "entry":
                existing_like = Like.objects.filter(
                    author=remote_author, entry=liked_object
                ).first()
            elif object_type == "comment":
                existing_like = Like.objects.filter(
                    author=remote_author, comment=liked_object
                ).first()
    ...
    def _handle_comment(self, recipient, data, remote_node):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {"message": "Entry not found"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            temp_url = (
                f"{remote_author.host}comments/{remote_author.id}/{entry.id}/temp"
            )
    ...
    def _handle_post(self, recipient, data, remote_node):
        ...
        try:
            ...
            # Generate a temporary URL first, then update it after creation
            temp_url = post_data.get(
                "id", f"{remote_author.host}posts/{remote_author.id}/temp"
            )

            # Check if there's image data
            image_data = None
            if post_data.get("image") and post_data.get("contentType", "").startswith(
                "image/"
            ):
                import base64
                import re
            ...
            print(
                f"DEBUG: Remote author: {remote_author.username} from node: {remote_author.node.name if remote_author.node else 'Unknown'}"
            )
            ...
            print(
                f"DEBUG: Created entry - ID: {entry.id}, Visibility: {entry.visibility}, Author: {remote_author.username}"
            )
            print(
                f"DEBUG: Entry is from remote node: {remote_author.node.name if remote_author.node else 'Unknown'}"
            )
            ...
            if entry.visibility != Entry.PUBLIC:
                ...
                print(
                    f"DEBUG: Created inbox item for {entry.visibility} post from {remote_author.username}"
                )
            else:
                print(
                    f"DEBUG: PUBLIC post from {remote_author.username} added to explore/feed (no inbox item)"
                )
                # Verify it's queryable
                from app.models import Entry as EntryModel

                found = EntryModel.objects.filter(
                    id=entry.id, visibility="PUBLIC"
                ).exists()
...
class InboxViewSet(viewsets.ModelViewSet):
    ...
    def read(self, request, pk=None):
        ...
        return Response({"message": "Item marked as read"}, status=status.HTTP_200_OK)
