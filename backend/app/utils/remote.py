...
def get_or_create_remote_entry(entry_data, source_node):
    """
    Get or create a remote entry from activity data.
    Prevents duplicate URL constraint violations.
    """
    from app.models import Entry
    from app.utils.remote import get_or_create_remote_author

    try:
        entry_url = entry_data.get("id") or entry_data.get("url")
        if not entry_url:
            logger.warning("Entry missing 'id' or 'url'")
            return None

        # Check if entry already exists
        existing_entry = Entry.objects.filter(url=entry_url).first()
        if existing_entry:
            logger.info(f"Remote entry already exists: {entry_url}")
            return existing_entry

        # Resolve author
        author_data = entry_data.get("author")
        if not author_data:
            logger.warning("Entry missing 'author'")
            return None

        remote_author = get_or_create_remote_author(author_data, source_node)
        if not remote_author:
            logger.warning("Failed to resolve remote author for entry")
            return None
        ...
        entry = Entry.objects.create(
            ...
            created_at=entry_data.get("published"),
        )

        logger.info(f"Created remote entry: {entry.url}")
        return entry

    except Exception as e:
        logger.error(f"Failed to create remote entry: {e}")
        return None
