...
class FederationService:
    ...
    def is_localhost_url(url: str) -> bool:
        ...
        try:
            ...
            return (
                netloc in ["localhost", "127.0.0.1", "::1"]
    ...
    def is_same_localhost_instance(url: str) -> bool:
        ...
        try:
            ...
            if FederationService.is_localhost_url(
                url
            ) and FederationService.is_localhost_url(settings.SITE_URL):
        ...
        try:
            ...
            if FederationService.is_localhost_url(url):
                ...
                localhost_nodes = Node.objects.filter(
                    host__icontains="localhost", is_active=True
                )
            ...
            node = Node.objects.filter(
                host__icontains=parsed.netloc, is_active=True
            ).first()
                ...
                try:
                    with transaction.atomic():
                        ...
                        if created:
                            ...
                            entry.content_type = entry_data.get(
                                "contentType", entry.content_type
                            )
                            entry.visibility = entry_data.get(
                                "visibility", entry.visibility
                            )
                            entry.description = entry_data.get(
    ...
    def process_inbox_item(
        ...
        try:
            ...
            if item_type == "like":
                ...
                return FederationService._process_follow_request(
                    recipient, data, remote_node
                )
            elif item_type in ["accept", "reject"]:
                return FederationService._process_follow_response(
                    recipient, data, remote_node
                )
            else:
    ...
    def _process_follow_response(
        ...
        try:
            ...
            try:
                ...
                follow.status = (
                    Follow.ACCEPTED if data.get("type") == "accept" else Follow.REJECTED
                )
                follow.save()
    ...
    @staticmethod
    def _get_or_create_remote_author(
        actor_data: Dict[str, Any], remote_node: Node
    ) -> Tuple[Author, bool]:
