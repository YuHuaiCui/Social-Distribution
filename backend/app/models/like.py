...
class Like(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        Remote likes must include a URL in advance.
        """
        # Ensure a UUID is present before building the URL
        if not self.id:
            self.id = uuid.uuid4()

        # Generate URL if it's a local like and missing
        if not self.url:
